...
namespace GNFS.GNFS
{
    public class Snfs
    {
        ...
        public Snfs(SpecialNumber number,IPolynomialGenerator generator)
        {
            ...
            var exp = Math.Pow(8 / 9.0, 1 / 3.0) * Math.Pow(BigInteger.Log(number.Value()), 1 / 3.0) * Math.Pow(Math.Log(BigInteger.Log(number.Value())), 2 / 3.0);


            _rationalPrimeBound = Convert.ToInt64(Math.Exp(exp)/10);
            _algebraicPrimeBound = _rationalPrimeBound;
            _kerDim = 10;
            _sieveSize = 5000000;
            _quadraticCharFbSize = Convert.ToInt64(3 * BigInteger.Log10(number.Value()));
        }

        public Snfs(SpecialNumber number, IPolynomialGenerator generator, long rfb, long afb, long sieveSize)
        {
            _number = number;
            _generator = generator;
